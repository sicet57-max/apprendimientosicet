...
# --- 2. CARGAR TODOS LOS DATOS (¡¡CORREGIDO!!) ---
def load_all_data():
    ...
    try:
        ...
        if not df_personal.empty:
            ...
            col_nombre_pers = [col for col in df_personal.columns if col.lower().startswith("nombre del t")][0]
            df_personal[col_nombre_pers] = df_personal[col_nombre_pers].astype(str).str.strip()
        
        if not df_comentarios.empty:
            df_comentarios.columns = df_comentarios.columns.astype(str).str.strip().str.lower()
...
lista_empleados = [""] + sorted(list(df_personal[col_nombre_tecnico].unique())) 
...
if empleado_sel:
    ...
    try:
        ...
        st.write(f"**Ubicación:** {info_empleado.get('Ubicación', 'N/A')}")
        st.write(f"**Número de Contacto:** {info_empleado.get('Número de contacto', 'N/A')}") 
    ...
    if df_calculos.empty:
        st.info("No hay datos de rendimiento numérico registrados.")
    else:
        ...
        if df_calc_empleado.empty:
            ...
            columnas_a_mostrar = [col for col in columnas_limpias if col in df_calc_empleado.columns]
            st.dataframe(df_calc_empleado[columnas_a_mostrar])
    ...
    if df_comentarios.empty:
        ...
        try:
            cedula_empleado = info_empleado.get("Cédula", "N/A")
            
            info_comentarios = df_comentarios[df_comentarios['cédula'] == cedula_empleado]
