...
# --- 4. LÓGICA DE CÁLCULO Y GUARDADO ---
if submitted:
    with st.spinner("Calculando y guardando..."):
        ...
        try:
            ...
            if df_calculos.empty:
                encabezado = [
                    "fecha", "mes", "dia", "nombre del técnico", "cédula", 
                    "empresa", "porcentaje_rendimiento", "comentario", 
                    "fecha_creacion" 
                ]
                gc.append_row_to_sheet(sheet_key="calculo", data_key="calculos", data_list=encabezado)
...
# --- 5. SECCIÓN DE HISTORIAL Y GESTIÓN ---
st.divider()
...
if df_calculos.empty:
    ...
    if df_filtrado.empty:
        ...
        # Definimos las columnas que SÍ queremos ver
        columnas_limpias = [
            'fecha', 
            'mes', 
            'dia', 
        ...
        # Filtramos la lista para quedarnos solo con las que existen
        columnas_a_mostrar = [col for col in columnas_limpias if col in df_filtrado.columns]
        
        # Mostramos el dataframe SOLAMENTE con esas columnas
        st.dataframe(df_filtrado[columnas_a_mostrar])
        # --- FIN DE LA CORRECCIÓN ---

        try:
            df_filtrado_grafica = df_filtrado.copy() 
            df_filtrado_grafica['Porcentaje (num)'] = df_filtrado_grafica['porcentaje_rendimiento'].astype(str).str.replace('%', '', regex=False).astype(float)
            fig = px.bar(
                df_filtrado_grafica,
                x="fecha", y="Porcentaje (num)", color=col_nombre_hist,
                title="Rendimiento por Fecha (Filtrado)", text="Porcentaje (num)",
            ...
            if modify_submitted:
                with st.spinner("Guardando modificación..."):
                    try:
                        ...
                        df_final_para_guardar = df_calculos_con_id.drop(columns=["id_unico_gestion"])
        ...
        if not ids_a_borrar_sel:
            ...
            with st.spinner("Eliminando registros..."):
                try:
                    ...
                    df_final_para_guardar = df_sin_borrados.drop(columns=["id_unico_gestion"])
                    gc.update_dataframe_in_sheet(sheet_key="calculo", data_key="calculos", df=df_final_para_guardar)
...
if df_calculos.empty:
    ...
    df_reporte = df_calculos.copy()
    try:
        df_reporte['Porcentaje (num)'] = df_reporte['porcentaje_rendimiento'].astype(str).str.replace('%', '', regex=False).astype(float)
    except Exception as e:
        st.error(f"Error al convertir porcentajes para el reporte: {e}")
        st.stop()
        
    df_summary = df_reporte.groupby([col_nombre_hist, col_mes_hist])['Porcentaje (num)'].mean().reset_index()
    df_summary = df_summary.rename(columns={'Porcentaje (num)': 'Rendimiento Promedio (%)'})
