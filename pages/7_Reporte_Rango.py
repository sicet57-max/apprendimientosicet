...
st.set_page_config(page_title="Reporte por Rango", page_icon="📅", layout="wide")
...
def load_calculos_data():
    try:
        ...
        if df_calculos.empty: return pd.DataFrame()
        ...
        if 'porcentaje_rendimiento' in df_calculos.columns:
            ...
            st.error("No se encontró 'porcentaje_rendimiento'.")
            return pd.DataFrame()
...
with col1:
    nombres_sel = st.multiselect("Filtrar por Técnico (Opcional):", options=df_calculos[col_nombre_hist].unique())
with col2:
    cedulas_sel = st.multiselect("Filtrar por Cédula (Opcional):", options=df_calculos[col_cedula_hist].unique())
...
# --- SECCIÓN DE HISTORIAL ELIMINADA DE ESTA PÁGINA ---