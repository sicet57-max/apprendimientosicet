...
def load_history_data():
    ...
    try:
        ...
        if not df_history.empty:
            ...
            df_history = df_history.reset_index(drop=True) # Aseguramos índice
        return df_history
    except Exception as e:
...
# --- ¡¡LÓGICA DE GUARDADO ACTUALIZADA (AHORA AÑADE AL HISTORIAL)!! ---
if st.button("Guardar este Reporte en Google Sheets", type="primary"):
    with st.spinner("Añadiendo reporte al historial en 'Reportes Personalizados'..."):
        try:
            # 1. Obtenemos el historial existente
            df_historial_existente = load_history_data()
            
            # 2. Combinamos el historial con el nuevo reporte
            df_para_guardar = pd.concat([df_historial_existente, df_reporte], ignore_index=True)
            
            # 3. Sobrescribimos la hoja con la lista combinada
            ...
            st.cache_data.clear() # Limpiamos el caché para que el historial se actualice
...
# --- 8. HISTORIAL DE REPORTES GUARDADOS (¡¡NUEVA SECCIÓN!!) ---
st.divider()
    ...
    if st.button("Confirmar Eliminación", type="primary"):
        if not ids_a_borrar_sel:
            ...
            with st.spinner("Eliminando reportes..."):
                try:
                    # Filtramos el DF original, quedándonos solo con las filas
                    # que NO están en la lista de borrado
                    df_sin_borrados = df_historial[
                        ~df_historial["id_unico_borrado"].isin(ids_a_borrar_sel)
                    ]
                    
                    # Quitamos la columna temporal 'id_unico_borrado'
                    df_final_para_guardar = df_sin_borrados.drop(columns=["id_unico_borrado"])
                    
                    # Subimos el nuevo DF (sin las filas borradas) a Google Sheets
                    gc.update_dataframe_in_sheet(
                        sheet_key="calculo",
                        data_key="reportes_rango",
