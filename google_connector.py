...
WORKSHEET_NAMES = {
    ...
    "calculo": {
        "calculos": "Hoja 1",
        "promedios": "Promedios Mensuales",
        "reportes_rango": "Reportes Personalizados" # La hoja que arregla el KeyError
    }
}
...
def update_dataframe_in_sheet(sheet_key, data_key, df):
    ...
    try:
        ...
        st.error(f"Error al actualizar la hoja '{worksheet_name_clean}': {e}")
        raise e
...
def append_row_to_sheet(sheet_key, data_key, data_list):
    ...
    try:
        ...
        st.error(f"Error al agregar fila en '{worksheet_name_clean}': {e}")
        raise e